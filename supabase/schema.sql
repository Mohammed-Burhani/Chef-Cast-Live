...
-- Announcements: Admin-published announcements (homepage "What's Cooking" + listing page)
...
-- Announcements policies (admin write, like recipes)
...
CREATE POLICY "Admins can insert announcements" ON announcements FOR INSERT TO authenticated
  WITH CHECK (EXISTS (SELECT 1 FROM profiles WHERE profiles.id = auth.uid() AND profiles.is_admin = true));
...
CREATE POLICY "Admins can delete announcements" ON announcements FOR DELETE TO authenticated
  USING (EXISTS (SELECT 1 FROM profiles WHERE profiles.id = auth.uid() AND profiles.is_admin = true));
...
-- Auto-bump announcements.updated_at on update
