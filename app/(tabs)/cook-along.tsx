/**
 * Live Quiz screen ΓÇö the core real-time game experience.
 *
 * State machine:
 *   idle  ΓåÆ  countdown (3-2-1)  ΓåÆ  question (timer)
 *         ΓåÆ  revealing (correct/wrong shown)
 *         ΓåÆ  between (mini leaderboard)
 *         ΓåÆ  (loop for each question)
 *         ΓåÆ  complete (final leaderboard)
 *
...
// ΓöÇΓöÇΓöÇ Phase screens ΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇ
...
function CountdownScreen() {
  ...
  return (
    <View style={[styles.fullCenter, { backgroundColor: colors.background }]}>
      ...
      <Text style={[styles.countdownSub, { color: colors.mutedForeground }]}>Quiz startingΓÇª</Text>
    </View>
  );
}
  ...
  return (
    <View style={[styles.betweenContainer, { backgroundColor: colors.background, paddingTop: topPad + 16, paddingBottom: bottomPad + 24 }]}>
      ...
      <Text style={[styles.betweenSub, { color: colors.mutedForeground }]}>
        {isLastQuestion ? "Final question! Leaderboard loading..." : `Next question in a momentΓÇª`}
      </Text>
            ...
            <View key={q.id} style={[styles.reviewCard, { backgroundColor: colors.surface }]}>
              ...
              <View style={styles.reviewAnswers}>
                ...
                {!answer?.selectedOptionId && (
                  <Text style={[styles.reviewYourAnswer, { color: colors.danger }]}>No answer ΓÇö time ran out</Text>
                )}
...
// ΓöÇΓöÇΓöÇ Root screen ΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇ

export default function QuizScreen() {
  const router = require('expo-router').useRouter();
  const { MOCK_EPISODES } = require('@/constants/mockData');

  // Redirect to live episode page instead of showing quiz directly in tab
  React.useEffect(() => {
    const liveEpisode = MOCK_EPISODES.find((ep: any) => ep.isLive);
    if (liveEpisode) {
      router.replace(`/episode/${liveEpisode.id}`);
    }
  }, []);

  // If no live episode, show idle
  return <IdleScreen />;
}
